...
public function do_upload_cake()
        {
                $this->load->model('Upload_model');
                $config['upload_path']          = './uploads/';
                $config['allowed_types']        = 'txt|jpg|csv';
                ...
                if ( ! $this->upload->do_upload('userfile'))
                {
                        $this->session->set_flashdata('error', 'Import failed!! Please check the file size,type and CSV structure!');
                        //$this->load->view('admin/muc', $error);
                        redirect(base_url() . 'index.php/admin/campaigns');
                }
                else
                {
                        $data = array('upload_data' => $this->upload->data());
                        ...
                        $filename_ac = $this->upload->data('full_path');
                        $this->Upload_model->insert_csv_cake($filename_ac);
                        $rows = $this->db->affected_rows();

                        $this->session->set_flashdata('success', 'Succesfully imported '.$rows.' Cake Campaigns!');
                        //$this->load->view('success', $data);
                        //unlink($filename_un);                                                                                 
                        redirect(base_url() . 'index.php/admin/campaigns');
                }
